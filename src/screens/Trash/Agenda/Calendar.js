...
import { useNavigation } from '@react-navigation/native'; // Utilisation du hook
...
const Agenda2 = ({ items, loadItems, main }) => {
    ...
    const renderItem = (item) => {
        ...
        return (
            <TouchableOpacity onPress={() => navigation.navigate('CreateTask', { isEdit: true, title: 'Modifier la tâche', DateId: item.date, TaskId: item.id, onGoBack: main.refreshItems })} style={styles.item}>
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            />
            <MyFAB onPress={() => navigation.navigate('CreateTask', { onGoBack: main.refreshItems })} />
        </View>
    );
};
